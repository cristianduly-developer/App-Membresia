...
export async function POST(req: NextRequest) {
  ...
  if (!emp?.org_id) return NextResponse.json({ error: 'No se encontró tu organización.' }, { status: 404 })

  try {
    const r = await fetch(`${SAAS_URL}/api/mp-crear-suscripcion`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-internal-key': process.env.INTERNAL_API_KEY || '' },
      body: JSON.stringify({ org_id: emp.org_id, app_id: 'app-membresias', plan }),
    })
