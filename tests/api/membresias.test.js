...
const BASE = 'https://membresias.solucionesmdp.com.ar'
const SAAS = 'https://saas.solucionesmdp.com.ar'
const TOKEN = process.env.MEMB_TOKEN || ''
const EMAIL = process.env.MEMB_EMAIL || 'cristianduly@gmail.com'
...
test.describe('verificar-acceso', () => {
  ...
  test('con token válido retorna acceso y plan', async ({ request }) => {
    ...
    // Membresías retorna { esColab, acceso: { tiene_acceso, plan, ... } }
...
// ─── MAILS ──────────────────────────────────────────────────────────
test.describe('mails', () => {
  test('mail renovacion sin datos retorna error controlado', async ({ request }) => {
    const res = await request.post(`${BASE}/api/mail/renovacion`, { data: {} })
    expect(res.status()).toBeLessThan(500)
  })

  test('mail bienvenida socio sin datos retorna error controlado', async ({ request }) => {
    const res = await request.post(`${BASE}/api/mail/socio-bienvenida`, { data: {} })
    expect(res.status()).toBeLessThan(500)
  })
})
...
// ─── GENERAR DEMO ───────────────────────────────────────────────────
test.describe('generar-demo', () => {
  test('sin datos retorna error controlado', async ({ request }) => {
    const res = await request.post(`${BASE}/api/generar-demo`, { data: {} })
    expect(res.status()).toBeLessThan(500)
  })
})
...
test.describe('reportar-error (SaaS)', () => {
  test('acepta errores de app membresias sin explotar', async ({ request }) => {
    const res = await request.post(`${SAAS}/api/reportar-error`, {
      headers: { 'x-app-id': 'membresias', 'x-app-key': 'clave_invalida' },
      data: { mensaje: 'Test E2E membresias', nivel: 'info' }
    })
    expect(res.status()).toBeLessThan(500)
  })
})
